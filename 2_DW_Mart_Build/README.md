...
**Challenge:**
I started with raw job posting data sitting in flat CSV files inside a Google Cloud Storage bucket. In that format, the data was basically useless for fast, analytical queries. Anyone trying to analyze it couldn't easily answer critical questions like:

- Which skills are gaining traction or dying out over time?
- Where are the hiring hot spots by company and location?
- What do salary distributions actually look like across different roles and skill sets?

To fix this, I needed to build a data warehouse that could serve as a single source of truth—ensuring clean, consistent data across the board. On top of that, I needed to spin up specialized data marts to pre-aggregate the data for specific use cases. This keeps queries simple, saves computing resources, and speeds up performance for daily analysis.
...
├── 07_create_company_mart.sql     # Company hiring mart 
